...
const useMenuFilters = (menuItems: MenuItem[]) => {
...
const useHeroSlideshow = (menuItems: MenuItem[]) => {
  ...
  const fallbackBackgroundImages = [
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80',
      description: 'Crafted with passion'
    },
    {
      id: 'fallback-2',
      name: 'Fine Dining',
      image: 'https://images.unsplash.com/photo-1551218808-94e220e084d2?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80',
      description: 'Artistry meets flavor'
    },
    {
      id: 'fallback-3',
      name: 'Signature Creations',
      image: 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80',
      description: 'Innovation in every bite'
    }
  ];

  const backgroundImages = heroBackgroundImages.length > 0 ? heroBackgroundImages : fallbackBackgroundImages;
...
const useMenuCategories = (menuItems: MenuItem[]) => {
...
// Components
interface MenuCartToggleProps {
  isOpen: boolean;
  onToggle: () => void;
  itemCount: number;
}

const MenuCartToggle: React.FC<MenuCartToggleProps> = ({ isOpen, onToggle, itemCount }) => {
  return (
    <motion.button
      onClick={onToggle}
      ...
      whileTap={{ scale: 0.95 }}
      className={cn(
        "relative flex items-center gap-2 px-4 py-2 rounded-xl font-semibold transition-all shadow-lg touch-manipulation",
        isOpen
          ? "bg-amber-500 text-black"
          : "bg-black/60 backdrop-blur-sm text-cream border border-cream/20 hover:bg-black/80"
      )}
    >
...
const MenuHero: React.FC<MenuHeroProps> = ({
  backgroundImages,
  heroImageIndex,
  setHeroImageIndex,
  isHeroPaused,
            ...
            >
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  target.src = 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=2000&q=80';
                }}
              />
            </motion.div>
          )}
                ...
                <div className="relative">
                  ...
                  {searchQuery && (
                    <Button
                      ...
                      className="absolute right-2 top-1/2 transform -translate-y-1/2 h-8 w-8 p-0 text-cream/60 hover:text-cream hover:bg-amber-600/20 rounded-full"
                    >
                      <X className="w-4 h-4" />
                    </Button>
                  )}
        ...
        <div className="flex-1 container mx-auto px-4 py-8 transition-all duration-300">
          {loading ? (
            ...
            >
              ...
              <h3 className="text-3xl md:text-4xl font-serif font-bold text-amber-400 mb-6">
                No dishes found
              </h3>
              
              <p className="text-cream/80 max-w-xl mx-auto mb-10 text-lg leading-relaxed">
                {searchQuery ? (
                  <>
                    We couldn't find any dishes matching <span className="text-amber-400 font-semibold">"{searchQuery}"</span>.
                    <br />
                    <span className="text-amber-400/80">Try a different search term or explore our categories.</span>
                  </>
                  ...
                  <>
                    ...
                    <br />
                    <span className="text-amber-400/80">Please check back in a moment.</span>
                  </>
                )}
              </p>
                ...
                <div className="space-y-4">
                    ...
                    {selectedItem.rating && (
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-lg font-medium text-amber-400">{selectedItem.rating.toFixed(1)}</span>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedItem.spiceLevel && selectedItem.spiceLevel > 0 && (
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-cream/60 font-medium">Heat Level:</span>
